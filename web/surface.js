...
export function atmVarianceCurve(surface) {
  ...
  // Кривая из нуля или одной точки — это не кривая, а отсутствие данных, и
  // отдавать её нельзя. Молчаливое следствие было бы опасным: нулевая
  // накопленная дисперсия даёт нулевой множитель, траектории вырождаются в
  // прямую, и панель показала бы риск 0.00% у всех оферт сразу, то есть
  // порекомендовала бы самые близкие к рынку страйки как безрисковые.
  // Замер подтвердил: при деградации фида все 264 оферты получали нулевой риск.
  if (pts.length < 2 || !(pts[pts.length - 1].w > 0)) return null;
